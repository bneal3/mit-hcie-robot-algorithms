// FUNCTION: The most basic way to move a cube
...
// FUNCTION: Even more basic way to move a cube
boolean moveCube(int id, float tx, float ty) {
  int[] lr = cubes[id].aim(tx, ty);
  float left = (lr[0] * .5);
  float right = (lr[1] * .5);
  int duration = (0);
...
boolean moveCubeSpeed(int id, float tx, float ty) {
  float dd = cubes[id].distance(tx, ty) / 100.0;
  // println("dd is: " + dd);
  dd = min(dd, 1);
...
// FUNCTION: Helper functions to drive the cubes
boolean rotateCube(int id, float ta) {
  float diff = ta - cubes[id].deg;
  if (diff > 180) diff -= 360;
  if (diff < -180) diff += 360;
  if (abs(diff) < 6) return true;
  int dir = 1;
